...
$JAMAICAC -useTarget $TARGETPLATFORM -d objs -classpath "$NDDSHOME/lib/java/nddsjava.jar":"$NDDSHOME/lib/java/distlog.jar":"$NDDSHOME/resource/app/lib/java/log4j-api-2.17.1.jar":"$NDDSHOME/resource/app/lib/java/log4j-core-2.17.1.jar" $ALL_SRC

echo "Generating code for Jamaica target $TARGETPLATFORM and RTI target $RTI_EXAMPLE_ARCH ..."
$JAMAICABUILDER -classpath objs:"$NDDSHOME"/lib/java/nddsjava.jar:"$NDDSHOME"/lib/java/distlog.jar:"$NDDSHOME"/resource/app/lib/java/log4j-api-2.17.1.jar:"$NDDSHOME"/resource/app/lib/java/log4j-core-2.17.1.jar -includeJar="$NDDSHOME"/lib/java/nddsjava.jar:"$NDDSHOME"/lib/java/distlog.jar:"$NDDSHOME"/resource/app/lib/java/log4j-2.17.1.jar -XlibraryPaths="$NDDSHOME"/lib/${RTI_EXAMPLE_ARCH} -XstaticLibraries="nddsjavaz" -setGraphics=none -interpret -target=$TARGETPLATFORM -destination=com.rti.hello.Hello com.rti.hello.Hello
