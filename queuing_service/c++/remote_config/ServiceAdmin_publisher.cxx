...
#include "ndds/ndds_requestreply_cpp.h"
  ...
  #define RTI_STRNCPY(dst, src, n) strcpy_s(dst, n, src)
#else
  #define RTI_STRNCPY(dst, src, n) strncpy(dst, src, n)
#endif
    ...
    void run_example(
        ...
        std::string sampleSelector) {
            ...
            */
            ...
            /* Send the request */           
            request.data().application_name = DDS_String_dup(targetService.c_str());
            ...
            if (action == RTI::Service::Admin::CommandActionKind_CREATE_ACTION) {
            ...
            std::cout << "		target service: " << request.data().application_name << std::endl;
                ...
                */
                ...
                for (iterator it = replies.begin(); it != replies.end(); ++it) {
                    if (it->info().valid_data) {
                        ...
                        if (it->data().retcode == RTI::Service::Admin::CommandReplyRetcode_OK_RETCODE) {
                            std::cout << "\nCommand returned: " << it->data().string_body << std::endl;
                            if (action == RTI::Service::Admin::CommandActionKind_GET_ACTION){
                        ...
                        if ((it->info().flag & (int) DDS::INTERMEDIATE_REPLY_SEQUENCE_SAMPLE) == 0){
...
void print_usage() {
    std::cout
        <<  "Usage: -d <domainId> -q <serviceName> -o <profileName> "
            "[create|delete|get|shutdown] <resourceIdentifier> {command options}"
        << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    RTI::Service::Admin::CommandActionKind action =
            RTI::Service::Admin::CommandActionKind_CREATE_ACTION;
    std::string resourceIdentifier =
    ...
    for (int i = 1; i < argc; ++i) {
        if (!strcmp(argv[i],"-d")) {
            ...
            RTI_STRNCPY(profileName, argv[++i], 128);
            ...
            *profileNamePtr = 0;
            ...
            if (!strcmp(argv[i], "create")) {
                resourceIdentifier =
                        "/domain_participant/DomainParticipant/shared_subscriber/SharedSubscriber";
                action = RTI::Service::Admin::CommandActionKind_CREATE_ACTION;
                ...
                action = RTI::Service::Admin::CommandActionKind_DELETE_ACTION;
            } else if (!strcmp(argv[i], "get")) {
                action = RTI::Service::Admin::CommandActionKind_GET_ACTION;
