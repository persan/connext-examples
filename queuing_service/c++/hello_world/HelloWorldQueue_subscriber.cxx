...
void subscriber_generateReaderGuidExpr(char * readerGuidExpr, int readerGuidExprLen)
{
    char * ptr = readerGuidExpr;
    
    RTI_STRNCPY(ptr, "@related_reader_guid.value = &hex(", readerGuidExprLen);
    ptr+= strlen(ptr);
    RTI_SNPRINTF(
            ptr,
            readerGuidExprLen - strlen(readerGuidExpr),
            "%032llx",
            (long long)readerGuidExpr);
    ptr+= strlen(ptr);
    RTI_STRNCPY(ptr, ")", readerGuidExprLen - strlen(readerGuidExpr));
}
...
int subscriber_main(HelloWorldParams & params)
{
    ...
    subscriber_generateReaderGuidExpr(readerGuidExpr, 255);
