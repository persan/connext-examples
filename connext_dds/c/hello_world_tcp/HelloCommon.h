...
static RTIBool parseCmdline(struct CmdLineArgs *argOut, int argc, const char **argv) {
    ...
    for (i = 1; i < argc; ++i) {
        ...
        if (!RTI_STRNCMP(argv[i], "-s", 2) || 
            ...
            if (*ptr != '\0') {
                usage(appName);
                fprintf(stderr, "! Value of --sampleCount argument is not "
                        "a number: %s\n", argv[i]);
                return RTI_FALSE;
            }
            continue;
...
DDS_ReturnCode_t configureTcpTransport(const struct CmdLineArgs * const args, 
    ...
    if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
        ...
        fprintf(stderr, "! unable to set property 'dds.transport.load_plugins': %d\n",
                rc);
    ...
    if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                ...
                DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
        fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.library': %d\n",
                rc);
        return DDS_RETCODE_ERROR;
    }
    ...
    if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                ...
                DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
        fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.create_function': %d\n",
                rc);
        return DDS_RETCODE_ERROR;
    }
    ...
    if (args->tls) {
        if (args->tlsCert == NULL) {
            fprintf(stderr, "! missing --tls-cert argument\n");
            return DDS_RETCODE_ERROR;
        }
        if (args->tlsChain == NULL) {
            fprintf(stderr, "! missing --tls-chain argument\n");
            return DDS_RETCODE_ERROR;
        }
        if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                    &part_qos->property,
                    ...
                    args->tlsCert,
                    DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
            fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.tls.verify.ca_file': %d\n",
                    rc);
            return DDS_RETCODE_ERROR;
        }
        if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                    ...
                    args->tlsChain,
                    DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
            fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.tls.verify.certificate_chain_file': %d\n",
                    rc);
            return DDS_RETCODE_ERROR;
        }
    }
    ...
    if (args->port) {
        ...
        if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                ...
                portstr,
                DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
            fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.server_bind_port': %d\n",
                rc);
            return DDS_RETCODE_ERROR;
        }
        /* Deprecated
        if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                    ...
                    "1",
                    DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
            fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.is_reachable': %d\n",
                    rc);
            return DDS_RETCODE_ERROR;
        }
        */
        if (args->addr != NULL) {
            ...
            if (args->tls) {
                if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                        ...
                        DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
                    fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.parent.classid': %d\n",
                        rc);
                    return DDS_RETCODE_ERROR;
                }
            } else {
            ...
            if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                    ...
                    args->addr,
                    DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
                fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.public_address': %d\n",
                    rc);
                return DDS_RETCODE_ERROR;
            }
            /*
    ...
    if (args->verbose > 0) {
        ...
        if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                ...
                "255",
                DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
            fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.logging_verbosity_bitmap': %d\n",
                rc);
            return DDS_RETCODE_ERROR;
        }
    }
    ...
    if ( (rc = DDS_PropertyQosPolicyHelper_assert_property(
                ...
                (args->asyncSend ? "1" : "0"),
                DDS_BOOLEAN_FALSE)) != DDS_RETCODE_OK) {
        fprintf(stderr, "! unable to set property 'dds.transport.tcp.tcp1.force_asynchronous_send': %d\n",
                rc);
        return DDS_RETCODE_ERROR;
    }
