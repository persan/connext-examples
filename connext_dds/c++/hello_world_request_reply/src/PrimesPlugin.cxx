    ...
    const struct DDS_TypeAllocationParams_t * alloc_params) 
{
    ...
    sample = new (std::nothrow) PrimeNumberRequest();
    if (sample == NULL) {
        return NULL;
    }
    ...
    if (!PrimeNumberRequest_initialize_w_params(sample,alloc_params)) {
        ...
        PrimeNumberRequest_finalize_w_params(sample, &deallocParams);
        delete  sample;
        sample=NULL;
    }
    return sample;
...
void PrimeNumberRequestPlugin_finalize_optional_members(
    PRESTypePluginEndpointData endpoint_data,
    PrimeNumberRequest* sample,
...
struct RTIXCdrInterpreterPrograms * PrimeNumberRequestPlugin_get_programs(void)
{
...
{ 
    ...
    PrimeNumberRequestPlugin_finalize_optional_members;
    ...
    const struct DDS_TypeAllocationParams_t * alloc_params) 
{
    ...
    sample = new (std::nothrow) PrimeNumberReply();
    if (sample == NULL) {
        return NULL;
    }
    ...
    if (!PrimeNumberReply_initialize_w_params(sample,alloc_params)) {
        ...
        the method will return before reach this point.*/
        /* coverity[uninit_use_in_call : FALSE] */
        PrimeNumberReply_finalize_w_params(sample, &deallocParams);
        delete  sample;
        sample=NULL;
    }
    return sample;
    ...
    unsigned int indent_level)
{
    ...
    if (DDS_LongSeq_get_contiguous_bufferI(&sample->primes) != NULL) {
        RTICdrType_printArray(
            ...
            (RTICdrUnsignedLong) DDS_LongSeq_get_length(&sample->primes),
            RTI_CDR_LONG_SIZE,
...
void PrimeNumberReplyPlugin_finalize_optional_members(
    PRESTypePluginEndpointData endpoint_data,
    PrimeNumberReply* sample,
    RTIBool deletePointers)
{
...
struct RTIXCdrInterpreterPrograms * PrimeNumberReplyPlugin_get_programs(void)
{
    return ::rti::xcdr::get_cdr_serialization_programs<
    PrimeNumberReply, 
    true, true, true>();
...
{ 
    ...
    plugin->finalizeOptionalMembersFnc =
    (PRESTypePluginFinalizeOptionalMembersFunction)
    PrimeNumberReplyPlugin_finalize_optional_members;
