...
/**
* A sequence of HelloWorldTopicQuery instances.
*/
public final class HelloWorldTopicQuerySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public HelloWorldTopicQuerySeq() {
        super(HelloWorldTopicQuery.class);
    }

    public HelloWorldTopicQuerySeq (int initialMaximum) {
        super(HelloWorldTopicQuery.class, initialMaximum);
    }

    public HelloWorldTopicQuerySeq (Collection<?> elements) {
        super(HelloWorldTopicQuery.class, elements);
    }

    public  HelloWorldTopicQuery get(int index) { 
        return (HelloWorldTopicQuery) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, HelloWorldTopicQuery.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' HelloWorldTopicQuery objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(HelloWorldTopicQuery.create());
