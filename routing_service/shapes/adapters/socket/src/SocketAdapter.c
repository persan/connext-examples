...
/*****************************************************************************/
#define RTI_SOCKET_ADAPTER_SEM_NAME_LENGTH_MAX (36)
...
struct RTI_RoutingServiceSocketStreamReader {
    ...
    char _bufferFullSemName[RTI_SOCKET_ADAPTER_SEM_NAME_LENGTH_MAX + 1];
  #endif
};
...
int RTI_RoutingServiceSocketStreamReader_connect(
    ...
    if(!RTIOsapiSocket_getHostByName(
        &addr.sin_addr, host)) {
        RTI_RoutingServiceEnvironment_set_error(
    ...
    RTI_RoutingServiceEnvironment * env) 
{
    ...
    snprintf(
            stream_reader->_bufferFullSemName, 
            RTI_SOCKET_ADAPTER_SEM_NAME_LENGTH_MAX, 
            "/%p", 
            stream_reader);
    stream_reader->_bufferFullSem = sem_open(
            stream_reader->_bufferFullSemName, 
            O_CREAT | O_EXCL,
            S_IRUSR | S_IWUSR, /* mode: read + write*/
            0 /* value */);
    if (stream_reader->_bufferFullSem == SEM_FAILED) {          
        RTI_RoutingServiceEnvironment_set_error(
            env, 
    ...
    struct RTI_RoutingServiceSocketStreamReader * self)
{
  ...
  #else
    if (self->_bufferFullSem != NULL
            && self->_bufferFullSem != SEM_FAILED) {
        sem_close(self->_bufferFullSem);
        sem_unlink(self->_bufferFullSemName);
    }
